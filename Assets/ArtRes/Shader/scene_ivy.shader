Shader "scene/ivy"
{
	...
	[Header(specular)]
		[MaterialToggle(_IsHLight_ON)]_Toggle("IsHLight", Float) = 1
		_HLight_K("HLight_K(高光系数)", Range(0,0.01)) = 1
	...
	{	
		...
		#pragma multi_compile __ _IsHLight_ON
		// #pragma vertex vert
		ENDCG
		...
		pass {
			...
			fixed4 fragLig(v2f i) :SV_Target
			{
				...
				//半兰伯特光照的漫反射
				fixed halfdiff = dot(L, N) + (i.color.r - 0.5 * 4) -1 + atten;
				fixed diffStep = smoothstep(0, _ShadowSmooth, halfdiff - _ShadowThreshold);
